...
impl<'a> Workers<'a> {
  ...
  fn try_assist(&self, thread_index: usize) {
    ...
    loop {
      ...
      let (old_active_threads, current_index, count_claimed) = if task.work_two_sided {
        // Mark that this thread is working on the task and read the workstealing index.
        let (old_active_threads, i) = task.counters.fetch_add(1, 0, Ordering::Acquire);
        (old_active_threads, i, (i >> 16) + (i & 0xFFFF))
      } else {
        // Mark that this thread is working on the task and claim the first blocks
        let (old_active_threads, i) = task.counters.fetch_add(1, 1, Ordering::Acquire);
        (old_active_threads, i, i)
      };
  ...
  fn call_task(&self, task_shared: epoch::Shared<TaskObject>, task: &TaskObject, signal: EmptySignal, first_index: u32) {
    (task.function)(self, unsafe { &*Task::ptr_data(task_shared.as_raw()) }, LoopArguments{ workstealing_size: task.work_size, workstealing_index: task.counters.workstealing_index(), empty_signal: signal, first_index });
